...

{
	...
	temp->period = 50000; // En unidades de us.
	temp->count = 0;
...
int main(int argc, char *argv[])
{
	...
	err = pthread_create(&fourth_id, NULL, thread3, (void*) pt3);
	if (err != 0) {
	...
	pthread_mutex_lock(&lock);
	printf("\nThis code just ran for %d seconds, which is a #define parameter.\n\n", SECONDS_TO_RUN);
	...
	printf("Considering the offset of %d us, and the period of %d us, there were %d iterations of Thread 3.\n\n", pt3->offset, pt3->period, pt3->count);
	pthread_mutex_unlock(&lock);
